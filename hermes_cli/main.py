...
def _has_any_provider_configured(*, strict_profile_scope: bool = False) -> bool:
    ...
    # installations have a plain string); also covers custom endpoints kept in config.
    # For the "custom" provider (local/self-hosted endpoints), base_url alone is
    # sufficient — many local servers need no key. For registered api-key providers
    # (bifrost, openrouter, etc.), provider+base_url without an api_key in config
    # means the key must be in .env — if it isn't there, the provider is NOT
    # actually configured (install.sh / bifrost-plugins-bootstrap.ts set
    # provider+base_url at install time, before the user has entered any key).
    if isinstance(model_cfg, dict):
        ...
        if cfg_provider == "custom" or cfg_api_key:
            ...
            # Registered provider with no inline api_key — env vars were already
            # checked above. Fall through: provider set but no key found anywhere yet.
            pass
        elif cfg_base_url:
